...
if True or len(sys.argv) > 1 and (sys.argv[1] == '-t' or sys.argv[1] == '--test'):
    pass
    #for f_name in os.listdir('tests/test_preprints'):
    #    shutil.copy('tests/test_preprints/' + f_name, 'matching/preprints')
elif len(sys.argv) > 1:
    dois = []
    f = open(sys.argv[1], 'r')
    for line in f:
...
#    os.rename('results', 'results' + str(result_num))
#    result_num += 1

passed = True
if True:#sys.argv[1] == '-t' or sys.argv[1] == '--test':
